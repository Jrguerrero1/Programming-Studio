...
//--------------------Server Functions--------------------	
	...
	public void connect() throws IOException {
		...
		String open = server_in.readLine();
			if(open == "WELCOME"){
...
//--------------------Game Functions--------------------
	...
	public void gameLoop() throws IOException {
		...
		/*According to specification this message will be
		 * game board info. 
		 */
		server_msg = server_in.readLine();
		System.out.println("Sever in: " + server_msg);
			if(server_msg.contains("INFO")){
				//INFO message parsed below
				String[] INFO_parsed = server_msg.split("\\s");
				int board_size = 2*Integer.parseInt(INFO_parsed[1])+ 2; 
				int seeds = Integer.parseInt(INFO_parsed[2]);
				int move_time = Integer.parseInt(INFO_parsed[3]);
				int player = 0;
				if(INFO_parsed[4] == "S"){
					player = board_size/2;
				}
				
				int[] board_side = new int[Integer.parseInt(INFO_parsed[1])];
				
				/*Create an array for one side of board */
			if(INFO_parsed[5] == "R"){
				for(int i = 0; i < Integer.parseInt(INFO_parsed[1]); i++){
					board_side[i] = Integer.parseInt(INFO_parsed[6+i]);
				}	
			}			
		}
		//receive();
